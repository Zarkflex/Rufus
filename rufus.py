...
def main():
    ...
    port = int(Write.Input("Enter the port -> ", Colors.blue_to_red, interval=0.005))

    print()
    th = Write.Input("Enter threads (press enter for 1000) -> ", Colors.blue_to_red, interval=0.005)
    
    th = int(th) if th else 1000

    print('\n')

    if Rufus.check_port(ip, port):
        input(Col.cyan + f"{ip}:{port}" + Colorate.Horizontal(Colors.red_to_blue, " is opened! Press enter to start attack..."))
    else:
        input(Col.cyan + f"{ip}:{port}" + Colorate.Horizontal(Colors.red_to_blue, " is closed. Cannot start attack."))
        exit()
